...
[
    ...
    db.books.find().count();
    //db.books.find({},{rating:{"$gt":8}, name:1, id:0}).pretty();

 db.books.find({}).sort({rating:1}) .pretty();
   db.books.find({},{id:0,rating:1,name:1}).sort({name:1}).pretty();
   db.books.find({rating:{$gt:8}},{_id:0,rating:1,name:1}).sort({rating:-1}).limit(5).pretty();